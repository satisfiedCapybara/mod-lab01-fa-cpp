...
unsigned int faStr2(const char *str) {
    ...
    while (str[i] != '\0')
    {
        ...
        else if (inWord == true && uncorrectlyWord == false && isupper(str[i]))
        {
            uncorrectlyWord = true;
            count --;
        }
        else if (inWord == false && uncorrectlyWord == false && islower(str[i]))
        {
            inWord = true;
            uncorrectlyWord = true;
        }
        else if (inWord == false && uncorrectlyWord == false && isupper(str[i]))
        {
            inWord = true;
            count ++;
        }
...
unsigned int faStr3(const char *str) {
    ...
    return round(static_cast<double>(std::accumulate(begin(letters_count), end(letters_count), 0, std::plus<int>()))
                / letters_count.size());
}
